...
def test_get_MAVLink_location():
    ...
    initial_message = dialect.MAVLink_global_position_int_message(
        ...
        vx=0, vy=0, vz=0,  # Ground X, Y, Z Speed
        hdg=0  # Heading
    )
    mavlink.send(initial_message)
    assert loc_mavlink.location() == LatLng(10.0, 20.0), "Initial location did not match expected value"
    ...
    assert loc_mavlink.location() == LatLng(40.0, 50.0), "New location did not match expected value"
...
def test_get_MAVLink_altitude():
    ...
    altitude_message = dialect.MAVLink_global_position_int_message(
        ...
        relative_alt=0,  # Relative altitude in mm above the ground (not used in this test)
        vx=0, vy=0, vz=0,  # Ground X, Y, Z Speed (not used in this test)
        hdg=0  # Heading (not used in this test)
    )
    mavlink.send(altitude_message)
    
    # Check if the MAVLinkLocationProvider reports the altitude correctly
    assert loc_mavlink.altitude() == 15.0, "Altitude did not match expected value in meters"
...
def test_get_MAVLink_orientation():
    ...
    expected_orientation = Rotation(math.degrees(0.1), math.degrees(0.2), math.degrees(0.3))
...
def test_get_MAVLink_position_changes():
    ...
    # Verify new position
    assert loc_mavlink.location() == LatLng(40.0, 50.0), "New location did not match expected value"
    assert loc_mavlink.heading() == Heading(45.0), "New heading did not match expected value"
    assert loc_mavlink.altitude() == 2.0, "New altitude did not match expected value in meters"
