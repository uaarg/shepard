...
from src.modules.autopilot.altimeter import Altimeter, MovingAverage
...
class XM125(Altimeter):
    ...
    def mavlink_sensor_type(self) -> int:
        """Get the MAVLink sensor type (RADAR)"""
    ...
    def begin(self) -> bool:
        ...
        if not self._write_register(self.REG_START, self.min_distance_mm):
            if DEBUG:
                print("Failed to set start distance")
            return False

        if not self._write_register(self.REG_END, self.max_distance_mm):
            if DEBUG:
    ...
    def measure(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if peaks_with_average:
                self.consecutive_errors = 0
                self._latest_measurement = peaks_with_average
